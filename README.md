...
- **添加块**：点右下角 `+`，选择类型（个人信息 / 教育经历 / 工作经历 / 研究经历 / 项目 / 技能 / 自定义）
- **编辑**：每个块是一个表单，改动即时反映到中间预览
- **排序 / 复制 / 删除**：块标题右侧 `↑ ↓ ⋯`
- **导出 PDF**：`打印 / 导出 PDF`（或 `Ctrl+P`）→ 浏览器"另存为 PDF"。这是零依赖的 PDF 路径。⚠️ 打印对话框里请**取消勾选「页眉和页脚」**，否则会带出日期和页面标题（浏览器限制，CSS 无法去除）
- **Typst 高级导出**：`Typst ▾` → 「下载 .typ 源码」离线拿到排版源码（用 typst CLI 或 typst.app/playground 编译）；「导出 Typst PDF」在浏览器里用 WASM 编译（首次联网约 28MB）。适合想要更精致排版、且能接受联网/本地编译的用户
- **保存 JSON（Ctrl+S）**：Chrome/Edge 下第一次让你选一次保存位置（建议选项目文件夹），之后自动静默存到同一文件；Firefox 无此 API，退回普通下载
- **保存 / 备份**：数据自动存到浏览器 `localStorage`（按文件路径隔离）；建议常点 `导出` 备份
- **导入**：`导入`，支持应用 JSON 与 JSON Resume 两种格式
