...
- **自定义信息**：在「个人信息」里加「籍贯：江苏」「政治面貌：党员」这类自由键值对，显示在头衔下方。名称和内容都自己填，**没有预设类型**；留空的条目自动跳过。宽度不够时整条一起换行，不会把「名称：内容」拆开
- **照片**：在「个人信息」里勾选「显示照片」并选择图片 → 显示在 CV 右上角（30×40mm，3:4，自动居中裁切不变形）。勾了但还没选图时会显示一个**虚线占位框，并且会打印出来** —— 这是故意的：纸上看到框才知道自己忘了放照片。自动缩到 400px 内嵌进 JSON，不依赖外部文件
